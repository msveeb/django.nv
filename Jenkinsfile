        ...
        stage("integration") {
            steps {
                catchError(buildResult: 'SUCCESS', stageResult: 'UNSTABLE') {
                    sh "docker run -u root -v \$(pwd):/zap/wrk:rw --rm -t owasp/zap2docker-stable:2.10.0 zap-baseline.py -t https://prod-IaNqrvIh.lab.practical-devsecops.training -J zap-output.json"
                }
            }
            post {
                always {
